...
import { getPokemonBy } from '../../../actions/pokemons';
...
const PokemonScreen = ({ navigation, route }: Props) => {
  ...
  const { isLoading, data: pokemon } = useQuery({
    ...
    queryFn: () => getPokemonBy(pokemonId),
  ...
  return (
    <LinearGradient colors={pokemon.colors} style={{ height: '100%', width: '100%', position: 'absolute' }}>
      <ScrollView
        bounces={false}
      ...
      <Pressable style={{ position: 'absolute', top: top + 10, right: 20 }}
        onPress={() => navigation.pop()}
      >
        <IonIcon name='close-circle-sharp' color='white' size={40} />
      </Pressable>
    </LinearGradient>
  )
}
